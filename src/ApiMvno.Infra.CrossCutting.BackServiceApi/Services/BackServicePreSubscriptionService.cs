...
public class BackServicePreSubscriptionService : IBackServicePreSubscriptionService
{
    private readonly IBackServiceAuthService _backServiceAuthService;


    public IFlurlRequest Url => _backServiceAuthService.Url
        .AppendPathSegment("/PreSubscription");

    public BackServicePreSubscriptionService(IBackServiceAuthService backServiceAuthService)
    {
        _backServiceAuthService = backServiceAuthService;
    }

    public async Task<BackServiceBaseResponse<CreatePreSubscriptionResponse>> CreateAsync(CreatePreSubscriptionRequest request)
    {
        var response = new BackServiceBaseResponse<CreatePreSubscriptionResponse>();

        try
        {
            var result = await Url.PostJsonAsync(request);
            response = await result.GetJsonAsync<BackServiceBaseResponse<CreatePreSubscriptionResponse>>();
        }
